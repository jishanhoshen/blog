...
class Main_Container
{
	...
	{
		$this->main_container = $this->Main_Container($ds);
	}
	public function Main_Container($ds)
	{
	?>
<div class="main_container">
	...
	if (!empty($postResult)) {
		foreach ($postResult as $key => $post) {
			...
			$postReviewsql = "SELECT * FROM postReview where postId = '".$post['postId']."'";
			$postReviewresult = $conn->query($postReviewsql);
			if ($postReviewresult->num_rows > 0) {
			  while($postReviewrow = $postReviewresult->fetch_assoc()) {
			  	$postReview = $postReviewrow;
			  }
			}
	?>
	<div class="post <?php echo $themeMood?>">
		<div class="post_header <?php echo $themeMood?>">
			<div class="profile_box" style="background-image: url(img/<?php echo $user['profile'];?>);"></div>
		...
		<div class="post_body">
			<div class="post_container">
				<div class="post_text_area">
					...
					if (!$post['postTitle'] == "") {
						echo '
						<input id="copyTarget'.$post['postId'].'" value="http://localhost/blog/post.php?id='.$post['postId'].'" style="height:0;border:0;">
				...
				<div class="post_image_area">
					...
					if (!$post['postImage4'] == "") {
						...
						$postIdimage = "#postId1img".$postImageCount;
		...
		<div class="post_footer count_footer <?php echo $themeMood?>">
			...
			<div class="reacts_count">
				<div class="reacts">
					...
					<div class="react r-unlike" style="background-image: url(img/react/unlike.png);"></div>
					<?php }?>
					<?php if ($postReview['wowStatus'] == 1) {?>
					<div class="react r-unlike" style="background-image: url(img/react/wow.png);"></div>
					<?php }?>
					<?php if ($postReview['angryStatus'] == 1) {?>
					<div class="react r-unlike" style="background-image: url(img/react/angry.png);"></div>
					<?php }?>
					<?php if ($postReview['hahaStatus'] == 1) {?>
					<div class="react r-unlike" style="background-image: url(img/react/haha.png);"></div>
					<?php }?>
					<?php if ($postReview['loveStatus'] == 1) {?>
					<div class="react r-unlike" style="background-image: url(img/react/love.png);"></div>
					<?php }?>
					<?php if ($postReview['likeStatus'] == 1) {?>
					<div class="react r-unlike" style="background-image: url(img/react/like.png);"></div>
					<?php }?>
				</div>
